...
export function parseBankCsv(text) {
  ...
  lines.slice(0, 30).forEach((line, lineIndex) => {
    delimiters.forEach((delimiter) => {
      ...
      const indexes = {
        ...
        label: findColumn(headers, HEADER_NAMES.label),
        amount: findColumn(headers, HEADER_NAMES.amount),
        debit: findColumn(headers, HEADER_NAMES.debit),
        credit: findColumn(headers, HEADER_NAMES.credit),
      };
  ...
  const rows = lines
    ...
    .map((line, index) => {
      ...
      return {
        id: `csv-${index}`,
        label: indexes.label >= 0 ? cells[indexes.label] || "Opération importée" : "Opération importée",
