const HEADER_NAMES = {
  date: ["date", "date operation", "date d operation", "date de valeur", "booking date"],
  debitDate: ["date prelevement", "date de prelevement", "date debit", "date de debit", "debit date"],
...
function isDateLike(value) {
  return /^\s*\d{1,4}[\/.-]\d{1,2}[\/.-]\d{1,4}/.test(String(value ?? ""));
}

function parseHeaderlessDeferredCardRows(lines, delimiter) {
  return lines
    .map((line, index) => {
      const cells = splitCsvLine(line, delimiter);
      if (cells.length < 4 || !isDateLike(cells[0]) || !isDateLike(cells[1])) return null;
      const debit = parseNumber(cells[3]);
      const credit = parseNumber(cells[4]);
      const signedAmount = credit !== 0
        ? Math.abs(credit)
        : debit !== 0
          ? -Math.abs(debit)
          : 0;
      const type = signedAmount >= 0 ? "income" : "expense";
      const label = cells[2] || "Opération importée";
      ...
      return {
        ...
        ...(classification.budgetReason ? { reason: classification.budgetReason } : {}),
        confidence: classification.confidence,
        categoryReason: classification.reason,
        date: parseDate(cells[0]),
        debitDate: parseDate(cells[1]),
      };
    })
...
export function parseBankCsv(text) {
  ...
  lines.slice(0, 30).forEach((line, lineIndex) => {
    delimiters.forEach((delimiter) => {
      ...
      const indexes = {
        ...
        debitDate: findColumn(headers, HEADER_NAMES.debitDate),
  ...
  if (!bestHeader) {
    const inferredDelimiter = delimiters
      .filter((delimiter) => delimiterCount(lines[0] || "", delimiter) >= 3)
      .sort((a, b) => delimiterCount(lines[0] || "", b) - delimiterCount(lines[0] || "", a))[0];
    const headerlessRows = inferredDelimiter ? parseHeaderlessDeferredCardRows(lines, inferredDelimiter) : [];
    if (headerlessRows.length) {
      return { rows: headerlessRows, error: "" };
    }
    return {
      rows: [],
  ...
  const rows = lines
    ...
    .map((line, index) => {
      ...
      return {
        ...
        ...(indexes.debitDate >= 0 ? { debitDate: parseDate(cells[indexes.debitDate]) } : {}),
      };
    })
